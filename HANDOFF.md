...
| 6 (axis6) | 결정 대기 (지석님) | §6.6 | TBD | `.figma-cache/1713-271669_phase5_editor-1.api.json` |
...
| **5.3 axis5c** | 템플릿 저장 dialog + 헤더 메뉴 | **5.1 완료 후** | 신규 `TemplateSaveDialog.tsx` + `EditorHeader.tsx` 메뉴 영역 (5.1의 헤더 본체 변경과 분리되도록 5.1 종료 commit SHA 받고 시작) |
| **5.4 axis5d** | 전체보기 풀스크린 모드 | **5.1 완료 후** | `PreviewPanel.tsx` 의 `fullscreen` prop 추가 + 신규 `FullscreenOverlay.tsx` (필요 시) |
...
## 6.6 Phase 6 — 장면분석(세로) 화면 (결정 대기)

### 6.6.0 캐시 식별 결과
- Figma 이름: **"2-3.a 장면분석(세로)"** (노드 1713:271669)
- 캐시 파일명 `editor-1`이지만 실제는 **동영상 상세 그룹의 세로 변형**
- 핵심 텍스트: "94개 장면", 페이지네이션 1~5, 타임코드 `00:00:02` 등
- 기존 `VideoDetailPage` (Phase 1) 의 "장면 분석" 탭(`view === "scenes"`, `ScenesPanel`) 의 세로/모바일 버전일 가능성 ↑

### 6.6.1 지석님 결정 필요 (보류)
1. **세로 레이아웃의 의도**: 모바일 viewport 자동 전환? 명시적 토글 버튼? 별도 라우트(`/videos/[id]/portrait`)?
2. **우선순위**: Phase 1 보강(`axis1`)으로 묶을지, 독립 `axis6`으로 진행할지?
3. **모바일 viewport 검증**: 현재 dev 환경에서 모바일 시뮬레이션 가능한지?

### 6.6.2 결정 후 진행 안내
지석님이 (1) 의도 (2) 우선순위 결정해주면 head가 sub task로 분담.
