...
>
> **마지막 갱신**: 2026-05-15 (Phase 0 + Phase 1 완료, Phase 2/3/4 대기. 같은 PC + 다른 Claude 계정 병렬 작업 가정)
...
| Phase | 브랜치 | 핵심 spec 섹션 | 시작 파일 | Figma 캐시 |
|---|---|---|---|---|
| 2 | `feat/p3-axis2-wizard` | §4 (이 문서) | `services/web/src/features/shorts-auto-product-wizard/components/InlineWizardContainer.tsx` | `.figma-cache/1713-288216_phase2_wizard-criteria.api.json`, `...288103_phase2_wizard-indexing.api.json`, `...288149_phase2_wizard-product-single.api.json`, `...288182_phase2_wizard-product-multi.api.json` |
| 3 | `feat/p3-axis3-result` | §5 (이 문서) | `services/web/src/features/shorts-auto-product-wizard/components/ExportShortsButton.tsx` + WizardStepResult 컴포넌트 (find 필요) | `.figma-cache/1713-288042_phase3_wizard-result.api.json`, `...1602-36895_phase3_cancel-dialog.api.json` |
| 4 | `feat/p3-axis4-saved-shorts` | §6 (이 문서) | `services/web/src/app/shorts/page.tsx` | `.figma-cache/1713-287987_phase4_saved-shorts.api.json` |

### D. 진행 중 항상 지켜야 할 규칙
1. **Co-Authored-By Claude 금지** (Heimdex 저장소 정책)
2. **마이크로 커밋** — 컴포넌트 1개 또는 디자인 토큰 1세트 단위
3. **푸시 금지** — 사용자 명시 승인 필수
4. **백엔드 변경 0건** — 내보내기 API는 mock/stub만
5. **dev 서버 호출 금지** — `npm run dev` 은 지석님이 직접
6. **언어** — 한국어로 응답 (코드/식별자는 원어)
7. **figma MCP 사용 자제** — `.figma-cache/*.api.json` 우선. 캐시에 없는 정보만 MCP. (병렬 시 다른 세션이 사용 중일 수 있음)
8. **공유 파일 충돌 주의** — `tailwind.config.ts`, `globals.css`, `figma-index.ts` 수정 시 사용자에게 먼저 알림
...
- **Phase 1**: ✅ 완료 + 검증됨 (커밋 d2658a1, 2c109d4, +2개 추가)
  - 화자 A/B = red-h.400 / green-h.400 매핑 (`src/lib/speaker-transcript.ts`)
  - 탭 nav border/text = heimdex-navy
  - AI badge = bg-grayscale-100 / text-heimdex-navy-500
  - ScenesPanel "쇼츠 제작" 버튼 = primary navy
  - `AutoShortsCTA` = primary navy filled (outlined → filled)
  - type-check exit 0, vitest 330 pass / 0 fail
- **Phase 2/3/4**: 미시작. 빈 브랜치 생성됨
- **Figma 캐시**: ✅ 13개 노드 (전체 P3 디자인) `.figma-cache/*.api.json` 저장 완료
  - **REST API raw 응답** = 모든 레이아웃/색/폰트/auto-layout/텍스트 포함
  - 총 ~17MB. **로컬 전용** (`.gitignore`). `bash .figma-cache/fetch-nodes.sh` 로 누구나 재현
  - 같은 PC에 다른 Claude 계정 띄울 때는 이미 디스크에 있으므로 재실행 불필요
