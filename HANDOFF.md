...
>
> **마지막 갱신**: 2026-05-15 (Phase 0/1/2/4 완료, Phase 3 부분 진행)
...
- **Phase 0**: 완료 + 커밋됨 (커밋 9fe5d2e ~ 6f001c5)
- **Phase 1**: 완료 + 검증됨 (커밋 d2658a1, 2c109d4 등 — 화자 색/탭 nav/AI badge/AutoShortsCTA 등)
- **Phase 2**: ✅ 완료 (커밋 0226700, fd680a8, 7c10c68, 01fafdb, 7799b3d) — branch `feat/p3-axis2-wizard`
  - InlineWizardBreadcrumb → heimdex-navy stepper 토큰 (lucide ChevronRight, 20/600 라벨, Phase 0 Stepper와 동일 비주얼)
  - 3 inline selector (Distribution / Length / Count) → navy 활성 보더 + grayscale-100/500 비활성. CountSelector suggestion 정책은 기존 "10분당 1개" 유지 (Figma 카피의 "15:40 → 5~7개"는 designer prototype, 실제 코드 정책은 라이브커머스 VOD 기준 보수적)
  - InlineWizardCriteriaPanel → shadow-card + rounded-card on white, 다음 버튼 = 공통 `Button` 컴포넌트, 내부 section gap 60
  - InlineWizardProductPanel → shadow-card, 안내문구 single/multi 분기 (`최대 ${cap}개` 동적), 카드 그리드 2px navy 보더 + Lucide Check 체크박스, multi-mode cap 도달 시 `Snackbar` (auto-dismiss 2.5s, single은 silent no-op 유지)
  - IndexingProgressPanel 신규 (Figma 1713:288103) — 4 stage chip (enumerating→동영상 분석 / tracking→상품 인식 / assembling→장면 조립 / rendering→렌더링) + 큰 navy % + ETA. 결과 페이지 와이어업은 Phase 3에서. smoke test 5건 통과
  - 백엔드 컨트랙트/createScanOrder 시그니처/polling 로직 무변경
  - `npx tsc --noEmit` exit 0, wizard 전체 vitest exit 0
- **Phase 3**: 부분 진행 (커밋 37bf058, a0e9a90, f68cd13, bd2afb9) — branch `feat/p3-axis3-result`
  - CancelGenerationDialog (Figma 1602:36895), ResultStatusChip 4상태, ResultCardMenu(stage-driven), ResultCard 287×253
  - 헤더/페이지 통합 wiring 후속 작업 필요
- **Phase 4**: ✅ 완료 (커밋 fc101eb, 8779678) — branch `feat/p3-axis4-saved-shorts`
  - Pagination active 색 indigo → heimdex-navy-500
  - SavedShortsPage 전체 리디자인 (Figma 1713:287987):
    - 헤더: "저장된 쇼츠 목록 N개" (count navy) + "새 쇼츠 생성" secondary + "내보내기" primary
    - Searchbar (h=56) + 제목 client-side 필터
    - 정렬 dropdown (생성 일자 순 / 오래된 순) navy 강조
    - 9:16 카드 그리드 (200×337 aspect, rounded-card 10px, 4-col wrap, gap 16/24)
    - heimdex-navy 팔레트 + lucide 아이콘 (Search/Plus/Download/Trash2/Film/AlertCircle)
    - Export 진행 Snackbar (top-right, tone=loading) — useExportBatch 백엔드 wiring 자리는 `NOTE(export-backend-tbd)` 주석으로 표시 (HANDOFF §6.3)
  - 테스트 selector 보존 (`saved-shorts-render-edit-link`, `saved-shorts-summary`, `saved-shorts-generate-summary`)
  - `npx tsc --noEmit` exit 0
- **Figma 캐시**: 13개 노드 `.figma-cache/*.api.json` 로컬 저장 완료
