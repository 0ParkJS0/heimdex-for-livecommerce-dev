...
- **Figma 캐시**: ✅ 7개 노드 저장 완료 (`.figma-cache/<node-id>_<name>.json`)
  - **완전 raw TSX**: `1713-288216_wizard-criteria.json` (9KB)
  - **요약 + asset URL + variableDefs**: 나머지 6개 (각 1-3KB)
  - 모두 `designContext` (Figma 자동 생성 TSX 코드 또는 요약) + `variableDefs` (색상 토큰 맵) 포함
  - ⚠️ 다른 세션이 Phase 2-4 구현 시 캐시 + 본 HANDOFF spec 병행 권장. 캐시가 요약이라 픽셀 단위 좌표 필요 시 Figma MCP 직접 호출 (단일 세션 제약 주의)
...
### 14.1 파일 명명
`{nodeId-hyphen}_{slug}.json` (콜론은 하이픈으로 치환)

### 14.2 파일 내용 구조
```json
{
  "nodeId": "1713:288216",
  "fileKey": "PYDMMAvFq7PmhjyVTftiU6",
  "fetchedAt": "...",
  "designContext": "<Figma TSX code 또는 구조 요약 + asset URLs>",
  "variableDefs": {
    "Heimdex Navy/500": "#234c77",
    "Grayscale/800": "#272833",
    ...
  }
}
```
...
```bash
# 캐시 확인
cat .figma-cache/1713-288216_wizard-criteria.json | jq -r '.designContext'

# 토큰 추출
cat .figma-cache/1713-288216_wizard-criteria.json | jq '.variableDefs'
```

### 14.4 컬러 토큰 매핑 (cache → Tailwind)
Figma 변수 → Phase 0 Tailwind class:
- `Heimdex Navy/400` → `heimdex-navy-400`
- `Heimdex Navy/500` → `heimdex-navy-500`
- `Heimdex Navy/600` → `heimdex-navy-600`
- `Grayscale/{10|100|200|400|500|800}` → `grayscale-{...}`
- `Neutral/{50|100|...|800}` → `neutral-h-{...}`
