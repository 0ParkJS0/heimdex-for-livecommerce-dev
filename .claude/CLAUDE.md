...
**VLM Model**: Qwen2.5-VL-7B-Instruct-AWQ (4-bit quantized, ~6-7GB VRAM). Runs on Aircloud RTX 4070 Super (12GB). Engine key: `qwen2vl` (same class handles 2B and 7B via `DRIVE_CAPTION_MODEL` env var). AWQ models auto-detected by name and loaded with `device_map="auto"`.
...
Color picker in image search mode lets users find scenes by visual tone. Extracts 5 dominant colors per keyframe via k-means, builds a 27-dim HSL histogram (8 hue × 3 saturation + 3 achromatic bins), and searches via kNN in OpenSearch.

- **Extraction**: `color_extraction.py` — pure functions, no GPU, no scikit-learn. Pillow only.
- **Index fields**: `color_embedding` (knn_vector 27-dim), `dominant_colors` (keyword, display only)
- **Query flow**: Color picker sends `color_hex` param → `hex_to_color_histogram()` generates query vector → `search_color_vector()` kNN → 4th signal in weighted RRF fusion
...
- **Graceful degradation**: Color kNN returns empty results (not 500) when no color vectors exist yet
