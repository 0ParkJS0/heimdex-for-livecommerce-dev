...
def _run(message: dict):
    ...
    with patch("src.tasks.enumerate.ApiClient", return_value=api), \
        ...
        # ``_fetch_keyframes`` returns ``(scene_keyframes,
        # ocr_by_scene_id, file_name)``; mirror the shape so the overlay
        # path's ``ocr_by_scene_id.get(...)`` lookup resolves cleanly
        # and the grounding brand-detection has a filename source.
        fetch.return_value = (
            _one_keyframe(),
