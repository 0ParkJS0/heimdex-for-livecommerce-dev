...
Consumes the :class:`OverlayEnumerationResult` from
:mod:`shorts_auto_product.enumerate_overlay` and produces, per product,
a slot-assembled plan for a final short clip. The shorts segment is
the timeline in which the overlay was visible -- operator-declared
rather than inferred from BM25 / SAM2 / STT.
...
``boto3``, :mod:`app.config`,
:mod:`shorts_auto_product.enumerate_overlay` (sibling), and
