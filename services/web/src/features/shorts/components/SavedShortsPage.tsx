...
export function SavedShortsPage() {
  ...
  // NOTE(export-backend-tbd): mock progress wiring — once useExportBatch
  // is exposed at app level, replace the sequential loop below with the
  // batch hook's state map (see HANDOFF §6.3).
    ...
    <div className="mx-auto max-w-[943px] pt-4 font-pretendard">
      ...
      <div className="rounded-card border border-grayscale-100 bg-white p-[20px] shadow-card">
        ...
        <div className="flex items-center justify-between gap-[16px]">
          <h1 className="text-[20px] font-bold tracking-[-0.5px] text-neutral-h-800">
            저장된 쇼츠 목록{" "}
            <span className="text-heimdex-navy-500">{filtered.length}개</span>
          </h1>
        ...
        {isLoading ? (
          ...
          <div className="mt-[20px] grid grid-cols-2 gap-x-[16px] gap-y-[24px] sm:grid-cols-3 lg:grid-cols-4">
            {paged.map((item) => (
          ...
          <div className="mt-[20px] flex flex-col items-center justify-center py-16 text-neutral-h-400">
            <Film className="h-8 w-8" />
            <p className="mt-3 text-[14px]">
              {searchQuery ? "검색 결과가 없습니다." : "저장된 쇼츠 영상이 없습니다."}
            </p>
            {!searchQuery && (
              <>
                <p className="mt-1 text-[12px]">영상을 선택하고 자동으로 쇼츠를 만들어 보세요.</p>
                <Link
                  href="/"
                  className="mt-5 inline-flex items-center gap-[6px] rounded-[8px] border border-heimdex-navy-400 bg-white px-[16px] py-[8px] text-[13px] font-medium text-heimdex-navy-500 transition-colors hover:bg-heimdex-navy-500/5"
                >
                  영상 목록으로 이동
                </Link>
              </>
