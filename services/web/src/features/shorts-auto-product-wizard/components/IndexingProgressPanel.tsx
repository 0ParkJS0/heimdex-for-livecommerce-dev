...
//   rendering   → "렌더링"
//
// Mount condition (per HANDOFF §4.5) is owned by the caller (the wizard
// container / result page). This component takes the derived ``progress``,
...
const STAGES: ReadonlyArray<{ id: IndexingStage; label: string }> = [
  { id: "enumerating", label: "동영상 분석" },
  { id: "tracking", label: "상품 인식" },
  { id: "assembling", label: "장면 조립" },
  ...
  return (
    <div className="space-y-[20px] font-pretendard">
      ...
      <div className="space-y-[40px] rounded-card bg-white p-[20px] shadow-card">
        ...
        <ol
          className="flex items-stretch gap-[12px]"
          ...
          {STAGES.map((stage) => {
            ...
            return (
              <li
                key={stage.id}
                className={cn(
                  "flex flex-1 items-center justify-center gap-[8px] rounded-[10px] px-[16px] py-[12px] text-[14px] font-semibold tracking-[-0.35px] transition",
                  isCompleted &&
        ...
        <div className="flex flex-col items-center gap-[8px] pb-[20px] pt-[12px]">
          <p
            className="text-[48px] font-semibold leading-none tracking-[-1.2px] text-heimdex-navy-500"
