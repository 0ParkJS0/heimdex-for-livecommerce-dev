    ...
    >
      {marks.map((mark, idx) => {
        ...
        return (
          <span key={mark.ms}>
            <span
              // 2026-05-18 — align the label to the same vertical
              // center as the dot row by using the ``top-1/2 +
              // translate-y -50%`` trick instead of the earlier
              // ``items-center pt-[3px]`` hack. The Pretendard glyph
              // cap-height now matches the dot baseline exactly.
              className="absolute top-1/2 -translate-y-1/2 whitespace-nowrap text-[12px] font-medium leading-none tracking-[-0.3px] text-grayscale-800"
              style={{ left: mark.px }}
            >
              {mark.label}
            </span>
